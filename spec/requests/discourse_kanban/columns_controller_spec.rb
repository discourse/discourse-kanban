...
RSpec.describe DiscourseKanban::ColumnsController do
  fab!(:admin)
  fab!(:manager, :user)
  fab!(:outsider, :user)
  ...
  describe "POST /kanban/boards/:board_id/columns" do
    it "creates a column for board managers" do
      sign_in(manager)

      post "/kanban/boards/#{board.id}/columns.json", params: { column: { title: "Backlog" } }
    ...
    it "returns not found when the board does not exist" do
      ...
      post "/kanban/boards/0/columns.json", params: { column: { title: "Backlog" } }
    ...
    it "returns bad request for invalid payloads" do
      ...
      post "/kanban/boards/#{board.id}/columns.json", params: { column: { title: nil } }
  ...
  describe "PUT /kanban/boards/:board_id/columns/:id" do
    it "updates a column for board managers" do
      ...
      put "/kanban/boards/#{board.id}/columns/#{column.id}.json",
          params: {
            column: {
              title: "Renamed",
    ...
    it "returns not found when the column does not exist" do
      ...
      put "/kanban/boards/#{board.id}/columns/0.json", params: { column: { title: "Renamed" } }
    ...
    it "returns bad request for invalid payloads" do
      ...
      put "/kanban/boards/#{board.id}/columns/#{column.id}.json", params: { column: { title: nil } }

      expect(response.status).to eq(400)
    end
  end

  describe "DELETE /kanban/boards/:board_id/columns/:id" do
    it "deletes a column for board managers" do
      sign_in(manager)

      delete "/kanban/boards/#{board.id}/columns/#{column.id}.json"

      expect(response.status).to eq(204)
      expect(DiscourseKanban::Column.exists?(column.id)).to eq(false)
    end
    ...
    it "returns not found when the board does not exist" do
      ...
      delete "/kanban/boards/0/columns/#{column.id}.json"
    ...
    it "returns not found when the column does not exist" do
      ...
      delete "/kanban/boards/#{board.id}/columns/0.json"
