...
RSpec.describe Jobs::DiscourseKanban::CardPostProcess do
  fab!(:admin)
  fab!(:viewer, :user)
  fab!(:viewer_group, :group)
  fab!(:board) do
    Fabricate(:kanban_board, created_by: admin).tap do |board|
      Fabricate(
  ...
  describe "#execute" do
    ...
    it "stores the inline onebox data without changing the card timestamp" do
      ...
      DiscourseKanban::Action::CardInlineOnebox
        .expects(:call)
        .with(title: card.title)
        .returns(onebox_data)
    ...
    it "does not publish when resolving produces no data" do
      DiscourseKanban::Action::CardInlineOnebox.expects(:call).with(title: card.title).returns(nil)
    ...
    it "does nothing for a topic-backed card" do
      topic_card = Fabricate(:kanban_topic_card, board:, column:, created_by: admin)
      DiscourseKanban::Action::CardInlineOnebox.expects(:call).never
