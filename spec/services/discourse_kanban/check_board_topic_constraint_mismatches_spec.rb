...
RSpec.describe DiscourseKanban::CheckBoardTopicConstraintMismatches do
  ...
  describe ".call" do
    ...
    fab!(:board) do
      Fabricate(:kanban_board, created_by: acting_user, additional_manage_groups: [write_group])
    end
    ...
    context "when the target column belongs to another board" do
      fab!(:other_board, :kanban_board)
      fab!(:other_board_column) { Fabricate(:kanban_column, board: other_board) }
