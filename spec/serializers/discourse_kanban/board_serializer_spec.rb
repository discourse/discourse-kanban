...
RSpec.describe DiscourseKanban::BoardSerializer do
  fab!(:admin)
  fab!(:manager, :user)
  fab!(:writer, :user)
  ...
  fab!(:category)
  fab!(:visible_tag) { Fabricate(:tag, name: "visible-kanban") }
  fab!(:alpha_tag) { Fabricate(:tag, name: "alpha-kanban") }
  fab!(:hidden_tag) { Fabricate(:tag, name: "staff-kanban") }
  ...
  it "serializes core board attributes and nested columns" do
    ...
    Fabricate(:kanban_column, board:, title: "Backlog", position: 0, tag_id: visible_tag.id)
  ...
  it "serializes permission booleans from the scoped guardian" do
    ...
    writable_board = Fabricate(:kanban_board, created_by: admin)
    Fabricate(
  ...
  it "serializes the creator username and avatar template" do
    board = Fabricate(:kanban_board, created_by: admin)
  ...
  it "includes acl only when requested" do
    board = Fabricate(:kanban_board, created_by: admin, additional_manage_groups: [manage_group])
