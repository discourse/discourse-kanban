...
describe "Add topic to board from topic footer menu" do
  ...
  fab!(:product_board) { Fabricate(:kanban_board, name: "Product", column_names: %w[Backlog Done]) }
  fab!(:engineering_board) do
    Fabricate(:kanban_board, name: "Engineering", column_names: ["Doing"])
  end
  ...
  context "when logged in" do
    ...
    it "fixes constraints and updates the topic header through MessageBus" do
      ...
      Fabricate(:kanban_topic_card, board: engineering_board, column: existing_column, topic: topic)
