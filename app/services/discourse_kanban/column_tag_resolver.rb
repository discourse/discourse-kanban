...
module DiscourseKanban
  module ColumnTagResolver
    def self.resolve_tag_ids(current_tag_ids:, column:)
    ...
    def self.normalize_tag_ids(values)
      ...
      raise Discourse::InvalidParameters.new(
              I18n.t("discourse_kanban.errors.unknown_tag_ids", tag_ids: Array(values).join(",")),
            )
    end

    def self.existing_tag_ids(tag_ids)
      return [] if tag_ids.empty?

      Tag.where(id: tag_ids).pluck(:id)
    end
    private_class_method :existing_tag_ids
  end
end
