...
module("Integration | kanban topic pill menu", function (hooks) {
  ...
  test("clicking the pill of a topic on several boards opens a menu of them", async function (assert) {
    ...
    assert.dom(".kanban-topic-pill--multiple").hasAria("expanded", "false");

    await click(".kanban-topic-pill--multiple");

    assert.dom(".kanban-topic-pill--multiple").hasAria("expanded", "true");
    assert.deepEqual(
      [...document.querySelectorAll(".kanban-boards-menu__item-label")].map(
