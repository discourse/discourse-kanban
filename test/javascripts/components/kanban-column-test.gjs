...
import KanbanColumn from "discourse/plugins/discourse-kanban/discourse/components/kanban-column";
...
module("Integration | Component | KanbanColumn", function (hooks) {
  ...
  test("hides cards outside the recency window", async function (assert) {
    ...
    assert
      .dom(".discourse-kanban-column__show-all")
