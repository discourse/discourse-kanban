...
function topic(kanbanMemberships) {
  return EmberObject.create({ kanban_memberships: kanbanMemberships });
}
...
module("Discourse Kanban | Unit | Lib | kanban-topic-pill", function (hooks) {
  ...
  test("renders a single menu trigger carrying its boards when a topic is on several", function (assert) {
    ...
    assert
      ...
      .hasText("2 boards");
    assert.dom("a.kanban-topic-pill", el).exists({ count: 1 });
  ...
  test("lists the boards instead of a menu when the caller wants inert markup", function (assert) {
    ...
    assert.dom(".kanban-topic-pill--multiple", el).doesNotExist();
    assert.dom(".discourse-tags__tag-separator", el).exists({ count: 1 });
    assert.deepEqual(
      [...el.querySelectorAll("span.kanban-topic-pill")].map((pill) =>
