...
import {
  ...
  kanbanMotionEnabled,
} from "discourse/plugins/discourse-kanban/discourse/lib/kanban-motion";
...
module("Discourse Kanban | Unit | Lib | kanban-motion", function (hooks) {
  setupTest(hooks);
  ...
  test("kanbanMotionEnabled respects reduced motion", function (assert) {
    window.matchMedia = () => ({ matches: true });
    assert.false(kanbanMotionEnabled());

    window.matchMedia = () => ({ matches: false });
    assert.true(kanbanMotionEnabled());
  });
