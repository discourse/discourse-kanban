...
function pill(tagName, { extraClass, label, ...attributes }) {
  const className = ["kanban-topic-pill", "discourse-tag", "simple", extraClass]
    .filter(Boolean)
    .join(" ");
  ...
  return (
    ...
    `<span class="kanban-topic-pill__label">${escapeExpression(label)}</span>` +
    `</${tagName}>`
  );
}
...
function multiBoardPill(memberships) {
  return pill("a", {
    extraClass: "kanban-topic-pill--multiple",
    label: i18n("discourse_kanban.topic_pill.multiple", {
...
export function kanbanTagsHtml(topic, params) {
