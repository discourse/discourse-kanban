...
const dragToScroll = modifier((element) => {
  ...
  const recordSample = (event) => {
    ...
    const cutoff = event.timeStamp - DRAG_SCROLL_VELOCITY_WINDOW_MS;
  ...
  const startMomentum = (endTime) => {
    ...
    const cutoff = endTime - DRAG_SCROLL_VELOCITY_WINDOW_MS;
    while (velocitySamples.length > 1 && velocitySamples[0].time < cutoff) {
      velocitySamples.shift();
    }
    ...
    let velocity = -pointerVelocityPerMs * 16.67;
    velocity = Math.max(
